...
## uv - Gerenciador de Pacotes e Ambiente Virtual para Python

Para configurar o ambiente virtual utilizando o uv, siga os passos abaixo:

1. Sincronize as dependências do projeto (na raiz do projeto):

```sh
uv sync
```

2. Para instalar novas dependências, utilize:
...
3. Inicialize o Alembic (caso necessário):
...
1. Na raiz do projeto, sincronize e inicie o ambiente virtual:
...
Para executar os testes, basta rodar os arquivos Python correspondentes no diretório **requests_api** ou depurar via VSCode:

```sh
python requests_api/create_registro_batch.py
python requests_api/create_registro.py
python requests_api/delete_registro.py
python requests_api/get_registro_by_id.py
python requests_api/get_registro_cidade.py
python requests_api/get_registros.py
python requests_api/search_by_visitados.py
```
